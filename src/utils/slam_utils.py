...
def get_loss_mapping(config, image, depth, viewpoint, opacity=None, initialization=False):
    if initialization:
        image_ab = image
    else:
        image_ab = (torch.exp(viewpoint.exposure_a)) * image + viewpoint.exposure_b
...
def get_loss_mapping_uncertainty(
    ...
    dynamic_weight: Optional[Tensor] = None,
    precomputed_uncertainty: Optional[Tensor] = None,
) -> Tuple[Tensor, Tensor]:
    ...
    # Predict uncertainty from features, or reuse a prediction from the mapper when
    # it also needs the uncertainty to build a dynamic reliability mask.
    if precomputed_uncertainty is None:
        features = viewpoint.features.to(device=rendered_img.device)
        uncertainty = uncertainty_network(features)
    else:
        uncertainty = precomputed_uncertainty
    ...
    if dynamic_weight is not None:
        if dynamic_weight.dim() == 2:
            dynamic_weight = dynamic_weight.unsqueeze(0)
        if dynamic_weight.shape[-2:] != weights.shape[-2:]:
            dynamic_weight = F.interpolate(
                dynamic_weight.unsqueeze(0),
                size=weights.shape[-2:],
                mode="bilinear",
                align_corners=False,
            ).squeeze(0)
        weights = weights * dynamic_weight.detach().clamp(0.0, 1.0)
