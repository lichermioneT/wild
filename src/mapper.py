...
class Mapper(object):
    ...
    @torch.no_grad()
    def _normalize_dynamic_evidence(
        self,
        evidence: torch.Tensor,
        valid_mask: torch.Tensor,
        quantile: float,
    ) -> torch.Tensor:
        """Robustly normalize residual evidence into [0, 1]."""
        if valid_mask.any():
            scale = torch.quantile(evidence[valid_mask], quantile).clamp(min=1e-6)
        else:
            scale = evidence.detach().mean().clamp(min=1e-6)
    ...
    def _get_dynamic_reliability(
        ...
        """
        Estimate a temporally smoothed static reliability map for a keyframe.

        High uncertainty, strong photometric residuals, and foreground depth conflicts
        are treated as dynamic evidence. The resulting static reliability is stored as
        an EMA on the Camera object so transient residuals do not flicker across mapping
        iterations.
        """
        if (
            not self.dynamic_filter_enabled
            or self.iterations_after_densify_or_reset
            < self.dynamic_filter_params.warmup_iters
        ):
            return None
        ...
        ref_depth = torch.from_numpy(viewpoint.depth).to(
            dtype=torch.float32, device=rendered_depth.device
        )[None]

        uncertainty = torch.clip(uncertainty.detach(), min=0.1) + 1e-3
        uncertainty = map_utils.resample_tensor_to_shape(uncertainty, (h, w))
        rgb_residual = torch.abs(rendered_img.detach() - gt_img).mean(dim=0)

        depth_conflict = torch.clamp(rendered_depth.detach() - ref_depth, min=0.0)
        depth_conflict = depth_conflict.squeeze(0)
        valid_depth = ref_depth.squeeze(0) > 0.01
        visible = opacity.detach().squeeze(0) > self.dynamic_filter_params.opacity_threshold
        valid = valid_depth & visible

        uncertainty_score = self._normalize_dynamic_evidence(
            uncertainty,
            visible,
            self.dynamic_filter_params.uncertainty_quantile,
        )
        rgb_score = self._normalize_dynamic_evidence(
            rgb_residual,
            visible,
            self.dynamic_filter_params.rgb_quantile,
        )
        depth_score = self._normalize_dynamic_evidence(
            depth_conflict,
            valid,
        ...
        if hasattr(viewpoint, "dynamic_reliability"):
            ...
            viewpoint.dynamic_reliability = (
                ema * viewpoint.dynamic_reliability.to(static_weight.device)
                + (1.0 - ema) * static_weight
            ).detach()
        else:
    ...
    def initialize_map_opt(self):
        ...
        for mapping_iteration in range(self.init_itr_num):
            ...
            if not self.uncertainty_aware:
                ...
                current_uncertainty = self.uncer_network(
                    viewpoint.features.to(device=image.device)
                )
                dynamic_weight = self._get_dynamic_reliability(
                    viewpoint, image, depth, opacity, current_uncertainty.detach()
                )
                current_uncertainty, loss_init = get_loss_mapping_uncertainty(
                    ...
                    precomputed_uncertainty=current_uncertainty,
                )
    ...
    def map_opt_online(self, current_window, iters=1):
        ...
        for cur_iter in range(iters):
            ...
            if not self.uncertainty_aware:
                ...
                dynamic_weight = self._get_dynamic_reliability(
                    viewpoint, image_ab, depth, opacity, current_uncertainty.detach()
                )
                (
                    current_uncertainty,
